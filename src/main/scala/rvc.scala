...
object RVCExpander {
  ...
  def addi4spnImm(x: UInt) = Cat(x(10,8), x(12,11), x(5), x(6), UInt(0,2))
  ...
  def q1(x: UInt)(implicit p: Parameters) = {
    ...
    def addi16sp = {
      val opc = Mux(addiImm(x).orR, UInt(0x17), UInt(0x1F))
      Cat(addi16spImm(x), rd(x), UInt(0,3), rd(x), opc)
    }
    def lui = {
      val opc = Mux(addiImm(x).orR, UInt(0x37), UInt(0x3F))
      val insn = Cat(luiImm(x)(31,12), rd(x), opc)
      Mux(rd(x) === UInt(2) || rd(x) === UInt(0), addi16sp, insn)
    }
    def j = Cat(jImm(x)(20), jImm(x)(10,1), jImm(x)(11), jImm(x)(19,12), x0, UInt(0x67,7))
    def beqz = Cat(bImm(x)(12), bImm(x)(10,5), x0, UInt(0,3), bImm(x)(4,1), bImm(x)(11), UInt(0x63,7))
    def bnez = beqz | UInt(1 << 12)
    def arith = {
      def srli = Cat(shamt(x), rs1p(x), UInt(5,3), rs1p(x), UInt(0x13,7))
      def srai = srli | UInt(1 << 30)
      def andi = Cat(addiImm(x), rs1p(x), UInt(7,3), rs1p(x), UInt(13,7))
      def rtype = {
        ...
        Cat(rs2p(x), rs1p(x), funct, rd(x), opc) | sub
      }
  ...
  def q2(x: UInt)(implicit p: Parameters) = {
    ...
    def sdsp = Cat(sdspImm(x) >> 5, rs2(x), sp, UInt(3,3), sdspImm(x)(4,0), UInt(0x23))
    def swsp = Cat(swspImm(x) >> 5, rs2(x), sp, UInt(2,3), swspImm(x)(4,0), UInt(0x23))
    def fsdsp = Cat(sdspImm(x) >> 5, rs2(x), sp, UInt(3,3), sdspImm(x)(4,0), UInt(0x27))
    def fswsp = {
      if (p(XLen) == 32) Cat(swspImm(x) >> 5, rs2(x), sp, UInt(2,3), swspImm(x)(4,0), UInt(0x27))
      else sdsp
    }
    def jalr = {
      ...
      val ebreak = Cat(jr >> 7, UInt(0x73,7))
